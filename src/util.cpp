...
const char * const BITCOIN_CONF_FILENAME = "strength.conf";
const char * const BITCOIN_PID_FILENAME = "strengthd.pid";
 ...
 * by well-meaning strength adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "strength" is a composite category enabling all STRENGTH-related debug output
            if(ptrCategory->count(string("strength"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\STRENGTHCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\STRENGTHCore
    // Mac: ~/Library/Application Support/STRENGTHCore
    // Unix: ~/.strengthcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "STRENGTHCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/STRENGTHCore";
#else
    // Unix
    return pathRet / ".strengthcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty strength.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override strength.conf
